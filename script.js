...
/* =========================================================
   BUILDING SELECTORS - 3D & MAIN AR (REVISION 25)
========================================================= */
...
let current3DModelIndex = 0;

function modelButtonText(model, mode = "3d") {
  const modelsForBuilding = getModelsByBuilding(model.buildingId);
  if (modelsForBuilding.length > 1) {
    return mode === "ar" ? `Tampilkan AR ${model.label}` : `Tampilkan 3D ${model.label}`;
  }
  return mode === "ar" ? "Tampilkan AR" : "Tampilkan 3D Objek";
}

function renderModelChoiceButtons(buildingId, container, mode) {
  container.innerHTML = "";
  if (!buildingId) return;

  getModelsByBuilding(buildingId).forEach((model) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "button button-primary model-choice-button";
    button.textContent = modelButtonText(model, mode);
    button.addEventListener("click", () => {
      if (mode === "ar") openARModel(model.id);
      else open3DModel(model.id);
    });
    container.appendChild(button);
  });
}

function render3DModel(index) {
  const total = models3D.length;
  current3DModelIndex = (index + total) % total;
  const model = models3D[current3DModelIndex];

  $("#viewerTitle").textContent = model.title;
  $("#viewerCounter").textContent = `${current3DModelIndex + 1} / ${total}`;
  $("#viewerCard").classList.remove("hidden");

  if (!model.path) {
    $("#mainModelViewer").removeAttribute("src");
    $("#viewerMessage").textContent = `${model.title} belum tersedia.`;
    $("#mainModelViewer").classList.add("model-unavailable");
    return;
  }

  $("#viewerMessage").textContent = "";
  $("#mainModelViewer").classList.remove("model-unavailable");
  $("#mainModelViewer").setAttribute("src", model.path);
}

function open3DModel(modelId) {
  const index = models3D.findIndex((model) => model.id === modelId);
  if (index < 0) return;
  render3DModel(index);
  $("#viewerCard").scrollIntoView({ behavior: "smooth", block: "start" });
}

function openARModel(modelId) {
  const model = getModel3DById(modelId);
  if (!model) return;

  if (!model.path) {
    $("#arMessage").textContent = `Model AR ${model.title} belum tersedia.`;
    $("#arCard").classList.add("hidden");
    return;
  }

  $("#arMessage").textContent = "";
  $("#mainARViewer").setAttribute("src", model.path);
  $("#mainARStatus").textContent = `Siap menampilkan ${model.title}`;
  $("#arCard").classList.remove("hidden");
}

$("#viewerBuildingSelect").addEventListener("change", (event) => {
  $("#viewerMessage").textContent = "";
  renderModelChoiceButtons(event.target.value, $("#viewerModelActions"), "3d");
});

$("#arBuildingSelect").addEventListener("change", (event) => {
  $("#arMessage").textContent = "";
  renderModelChoiceButtons(event.target.value, $("#arModelActions"), "ar");
});

$("#previous3DModel").addEventListener("click", () => render3DModel(current3DModelIndex - 1));
$("#next3DModel").addEventListener("click", () => render3DModel(current3DModelIndex + 1));
